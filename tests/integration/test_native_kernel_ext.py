...
def test_features_lists_every_shipped_capability() -> None:
    assert set(helpers.native().FEATURES) >= {
        "predicate_skipping",
        "timestamp_travel",
        "table_changes",
        "files",
...
class TestFiles:
    def test_one_row_per_live_file(self, two_file_table: str) -> None:
        files = pa.table(helpers.snapshot(two_file_table).files())
    ...
    def test_paths_match_delta_rs(self, two_file_table: str) -> None:
        ...
        ours = set(pa.table(helpers.snapshot(two_file_table).files()).column("path").to_pylist())
    ...
    def test_partition_values_are_reported(self, partitioned: str) -> None:
        rows = pa.table(helpers.snapshot(partitioned).files()).to_pylist()
...
class TestPredicateSkipping:
    def test_stats_skip_a_file(self, two_file_table: str) -> None:
        ...
        assert pa.table(helpers.snapshot(two_file_table).files(pred)).num_rows == 1
    ...
    def test_scan_skips_files_but_does_not_filter_rows(self, two_file_table: str) -> None:
        ...
        got = pa.table(helpers.snapshot(two_file_table).scan(predicate=pred))
    ...
    def test_scan_with_projection_and_predicate_on_another_column(
        ...
        got = pa.table(helpers.snapshot(two_file_table).scan(columns=["id"], predicate=pred))
    ...
    def test_or_with_unconvertible_child_means_no_skipping(self, two_file_table: str) -> None:
        ...
        assert pa.table(helpers.snapshot(two_file_table).files(pred)).num_rows == 2
    ...
    def test_malformed_json_is_an_error(self, two_file_table: str) -> None:
        with pytest.raises(ValueError, match="not valid JSON"):
            helpers.snapshot(two_file_table).files("{nope")
    ...
    def test_partition_pruning(self, partitioned: str) -> None:
        ...
        rows = pa.table(helpers.snapshot(partitioned).files(pred)).to_pylist()
...
class TestTimeTravel:
    ...
    def test_timestamp_before_history_is_a_clear_error(self, two_file_table: str) -> None:
        with pytest.raises(ValueError, match="earliest recreatable commit"):
            helpers.snapshot(two_file_table, timestamp_ms=1000)

    def test_version_and_timestamp_are_exclusive(self, two_file_table: str) -> None:
        with pytest.raises(ValueError, match="not both"):
            helpers.snapshot(two_file_table, version=0, timestamp_ms=1000)
...
def cdf_table(tmp_path: Any) -> tuple[str, int]:
    ...
    helpers.snapshot(path).append(pa.table({"id": [1, 2], "g": ["a", "b"]}).to_reader())
    time.sleep(0.05)
    between = int(time.time() * 1000)
    time.sleep(0.05)
    helpers.snapshot(path).append(pa.table({"id": [3], "g": ["c"]}).to_reader())
...
class TestTableChanges:
    def test_full_feed(self, cdf_table: tuple[str, int]) -> None:
        path, _ = cdf_table
        rows = pa.table(helpers.native().table_changes(path)).to_pylist()
    ...
    def test_version_range_and_projection_keep_change_columns(
        ...
        path, _ = cdf_table
        table = pa.table(
            helpers.native().table_changes(path, start_version=2, end_version=2, columns=["id"])
        )
    ...
    def test_timestamp_bounds(self, cdf_table: tuple[str, int]) -> None:
        ...
        after = pa.table(helpers.native().table_changes(path, start_timestamp_ms=between))
        assert set(after.column("_commit_version").to_pylist()) == {2, 3}
        before = pa.table(helpers.native().table_changes(path, end_timestamp_ms=between))
    ...
    def test_start_version_and_timestamp_are_exclusive(self, cdf_table: tuple[str, int]) -> None:
        path, between = cdf_table
        with pytest.raises(ValueError, match="not both"):
            helpers.native().table_changes(path, start_version=1, start_timestamp_ms=between)

    def test_refuses_a_table_without_cdf(self, two_file_table: str) -> None:
        with pytest.raises(ValueError):
            pa.table(helpers.native().table_changes(two_file_table))
...
class TestRawMetadata:
    def test_metadata_json_is_a_protocol_metadata_action(self, two_file_table: str) -> None:
        ...
        md = json.loads(helpers.snapshot(two_file_table).metadata_json())
    ...
    def test_protocol_json_omits_feature_lists_below_table_features(
        ...
        proto = json.loads(helpers.snapshot(two_file_table).protocol_json())
    ...
    def test_protocol_json_includes_features_at_v3_v7(self, cdf_table: tuple[str, int]) -> None:
        proto = json.loads(helpers.snapshot(cdf_table[0]).protocol_json())
    ...
    def test_timestamp_is_the_commit_time_of_this_version(self, two_file_table: str) -> None:
        ...
        latest = helpers.snapshot(two_file_table).timestamp()
        first = helpers.snapshot(two_file_table, version=0).timestamp()
    ...
    def test_absent_domain_is_none_and_system_domains_are_readable(
        ...
        snap = helpers.snapshot(two_file_table)
    ...
    def test_clustering_domain_is_returned(self, tmp_path: Any) -> None:
        ...
        helpers.native().create_table(path, pa.schema([("id", pa.int64())]), cluster_by=["id"])
        config = helpers.snapshot(path).domain_metadata("delta.clustering")
...
class TestCommitRaw:
    def test_metadata_only_commit_is_visible_to_both_engines(self, two_file_table: str) -> None:
        ...
        assert (
            helpers.native().commit_raw(two_file_table, snap.version + 1, actions)
            == snap.version + 1
        )
        assert helpers.snapshot(two_file_table).table_properties()["custom.key"] == "v"
...
class TestPartitionedAppend:
    def test_rows_land_in_their_partitions(self, partitioned: str) -> None:
        ...
        version = helpers.snapshot(partitioned).append(
        ...
        ours = pa.table(helpers.snapshot(partitioned).scan()).sort_by("id").to_pydict()
        ...
        by_value = {
            ...
            for r in pa.table(helpers.snapshot(partitioned).files()).to_pylist()
    ...
    def test_typed_partition_values_are_serialised_by_the_protocol(self, tmp_path: Any) -> None:
        ...
        helpers.native().create_table(path, schema, partition_by=["day", "n"])
        data = pa.table(
            {
                "v": ["a", "b", "c"],
        ...
        helpers.snapshot(path).append(data.to_reader())

        parts = sorted(
            tuple(sorted(r["partition_values"]))
            for r in pa.table(helpers.snapshot(path).files()).to_pylist()
        )
        assert parts == [
            (("day", "2024-01-02"), ("n", "7")),
            (("day", "2025-06-30"), ("n", "8")),
    ...
    def test_missing_partition_column_is_refused(self, partitioned: str) -> None:
        with pytest.raises(ValueError, match="no such column"):
            helpers.snapshot(partitioned).append(pa.table({"id": [9]}).to_reader())


class TestUnityCatalogHelpers:
    def test_required_properties(self) -> None:
        props = helpers.native().uc_required_properties("tbl-123")
    ...
    def test_create_table_request_from_v0(self, tmp_path: Any) -> None:
        ...
        helpers.native().create_table(path, pa.schema([("id", pa.int64())]), properties={"k": "v"})
        body = json.loads(helpers.native().uc_create_table_request(path, "main.s.t"))
...
class TestCheckpoint:
    def test_writes_a_checkpoint_readable_by_both_engines(self, two_file_table: str) -> None:
        ...
        assert _with_timeout(lambda: helpers.snapshot(two_file_table).checkpoint()) is True
        ...
        assert pa.table(helpers.snapshot(two_file_table).scan()).num_rows == 5
    ...
    def test_second_checkpoint_reports_already_exists(self, two_file_table: str) -> None:
        _with_timeout(lambda: helpers.snapshot(two_file_table).checkpoint())
        assert _with_timeout(lambda: helpers.snapshot(two_file_table).checkpoint()) is False

    def test_checkpoint_on_partitioned_table_after_kernel_append(self, partitioned: str) -> None:
        helpers.snapshot(partitioned).append(pa.table({"id": [2], "region": ["us"]}).to_reader())
        assert _with_timeout(lambda: helpers.snapshot(partitioned).checkpoint()) is True
        assert pa.table(helpers.snapshot(partitioned).scan()).num_rows == 2
