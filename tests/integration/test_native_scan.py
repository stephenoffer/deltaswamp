...
class TestSnapshotMetadata:
    ...
    def test_schema_matches_what_was_written(self, table_path: str) -> None:
        schema = helpers.snapshot(table_path).schema()
    ...
    def test_agrees_with_delta_rs_on_version(self, table_path: str) -> None:
        ...
        assert helpers.snapshot(table_path).version == DeltaTable(table_path).version()
...
class TestScan:
    def test_exports_arrow_c_stream(self, table_path: str) -> None:
        ...
        assert hasattr(helpers.snapshot(table_path).scan(), "__arrow_c_stream__")

    def test_reads_all_rows(self, table_path: str) -> None:
        assert pa.table(helpers.snapshot(table_path).scan()).num_rows == 7

    def test_values_round_trip(self, table_path: str) -> None:
        got = pa.table(helpers.snapshot(table_path).scan()).to_pydict()
    ...
    def test_column_projection(self, table_path: str) -> None:
        assert pa.table(
            helpers.snapshot(table_path).scan(columns=["id", "amount"])
        ).column_names == [
            "id",
            "amount",
        ]
    ...
    def test_projection_of_unknown_column_errors(self, table_path: str) -> None:
        with pytest.raises((ValueError, OSError, RuntimeError)):
            helpers.snapshot(table_path).scan(columns=["nonexistent"])


class TestTimeTravel:
    def test_reads_an_earlier_version(self, table_path: str) -> None:
        assert pa.table(helpers.snapshot(table_path, version=0).scan()).num_rows == 5
    ...
    def test_version_beyond_the_log_errors(self, table_path: str) -> None:
        with pytest.raises((ValueError, OSError, RuntimeError)):
            helpers.snapshot(table_path, version=99)


class TestLogTailValidation:
    def test_non_contiguous_tail_is_refused(self, table_path: str) -> None:
