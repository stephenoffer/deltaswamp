...
def _kernel_available(monkeypatch: pytest.MonkeyPatch) -> None:
    """These are `supports()` decisions, not reads: they need no compiled kernel.

    CI's lint job runs the unit tests from source without the extension, where
    `available()` is False and every verdict short-circuits to "not installed".
    """
    import deltaswamp.engine.kernel as kernel

    monkeypatch.setattr(KernelEngine, "available", lambda self: True)
